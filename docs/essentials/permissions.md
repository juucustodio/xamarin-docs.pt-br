...
Para verificar o status atual de uma permissão, use o método `CheckStatusAsync` juntamente com a permissão específica para obter o status.
...
Para solicitar uma permissão dos usuários, use o método `RequestAsync` juntamente com a permissão específica para solicitar. Se o usuário concedeu permissão anteriormente e não o revogou, esse método retornará `Granted` immediatelly e não exibirá uma caixa de diálogo. 
...
Observe que, em algumas plataformas, uma solicitação de permissão só pode ser ativada uma única vez. Solicitações adicionais devem ser tratadas pelo desenvolvedor para verificar se um permissão está no estado `Denied` e pedir ao usuário para ligá-lo manualmente.

## <a name="permission-status"></a>Status da permissão

Ao usar `CheckStatusAsync` ou `RequestAsync` um `PermissionStatus` será retornado que será usado para determinar as próximas etapas.
...
Ao implementar uma permissão em uma plataforma específica, a classe `BasePlatformPermission` pode ser herdada. Isso fornece métodos adicionais de auxiliares de plataforma para verificar automaticamente as declarações.
...
As permissões devem ter uma cadeia de caracteres correspondente no arquivo de `Info.plist`. Quando uma permissão é solicitada e um pop-up não aparece mais, se você solicitar a permissão uma segunda vez. Você deve solicitar que o usuário ajuste manualmente a configuração na tela de configurações de aplicativos no iOS.
