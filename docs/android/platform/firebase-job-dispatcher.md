...
- Uma `Firebase.JobDispatcher.Constraint` é um valor opcional que descreve uma condição que deve ser atendida antes que o trabalho possa ser executado, como o dispositivo está em uma rede ou um carregamento não medido.
- O `Firebase.JobDispatcher.Job` é uma API que unifica as APIs anteriores no para uma unidade de trabalho que pode ser agendada pelo `JobDispatcher`. A classe `Job.Builder` é usada para instanciar uma `Job`.
- Uma `Firebase.JobDispatcher.JobDispatcher` usa as três APIs anteriores para agendar o trabalho com o sistema operacional e para fornecer uma maneira de cancelar trabalhos, se necessário.

Para agendar o trabalho com o Dispatcher do trabalho firebase, um aplicativo Xamarin. Android deve encapsular o código em um tipo que estende a classe `JobService`. `JobService` tem três métodos de ciclo de vida que podem ser chamados durante o tempo de vida do trabalho:

- **`bool OnStartJob(IJobParameters parameters)`** &ndash; esse método é onde o trabalho ocorrerá e deve ser sempre implementado. Ele é executado no thread principal. Esse método retornará `true` se houver trabalho restante ou `false` se o trabalho for feito. 
...
Para agendar um trabalho, o aplicativo criará uma instância de um objeto `JobDispatcher`. Em seguida, um `Job.Builder` é usado para criar um objeto `Job`, que é fornecido para o `JobDispatcher` que tentará e agendará o trabalho para ser executado.
...
Depois de adicionar a biblioteca do Dispatcher de trabalho do firebase, crie uma classe `JobService` e, em seguida, Agende-a para ser executada com uma instância do `FirebaseJobDispatcher`.

### <a name="creating-a-jobservice"></a>Criando um JobService

Todo o trabalho realizado pela biblioteca do dispatcher do trabalho firebase deve ser feito em um tipo que estenda a classe abstrata `Firebase.JobDispatcher.JobService`. Criar um `JobService` é muito semelhante a criar um `Service` com a estrutura do Android: 

1. Estender a classe de `JobService`
2. Decorar a subclasse com a `ServiceAttribute`. Embora não seja estritamente necessário, é recomendável definir explicitamente o parâmetro `Name` para ajudar na depuração do `JobService`. 
...
Antes que qualquer trabalho possa ser agendado, é necessário criar um objeto `Firebase.JobDispatcher.FirebaseJobDispatcher`. O `FirebaseJobDispatcher` é responsável por agendar um `JobService`. O trecho de código a seguir é uma maneira de criar uma instância do `FirebaseJobDispatcher`: 
...
No trecho de código anterior, o `GooglePlayDriver` é classe que ajuda o `FirebaseJobDispatcher` a interagir com algumas APIs de agendamento em Google Play Services no dispositivo. O parâmetro `context` é qualquer `Context`do Android, como uma atividade. Atualmente, a `GooglePlayDriver` é a única implementação de `IDriver` na biblioteca do Dispatcher de trabalhos do firebase. 

A associação do Xamarin. Android para o Dispatcher do trabalho firebase fornece um método de extensão para criar uma `FirebaseJobDispatcher` da `Context`: 

```csharp
FirebaseJobDispatcher dispatcher = context.CreateJobDispatcher();
```

Depois que o `FirebaseJobDispatcher` tiver sido instanciado, é possível criar um `Job` e executar o código na classe `JobService`. A `Job` é criada por um objeto `Job.Builder` e será discutida na próxima seção.
...
- O tempo de _vida_ de um `Job`(quanto tempo será agendado para ser executado) é apenas até que o dispositivo seja reinicializado &ndash; depois que o dispositivo reinicializar o `Job` seja perdido.
- Um `Job` não é recorrente &ndash; só será executado uma vez.
- Um `Job` será agendado para ser executado assim que possível.
- A estratégia de repetição padrão para uma `Job` é usar uma _retirada exponencial_ (discutida em mais detalhes abaixo na seção [definindo um RetryStrategy](#Setting_a_RetryStrategy))
...
- `FirebaseJobDispatcher.ScheduleResultNoDriverAvailable` &ndash; um `IDriver` inválido foi usado ou o `IDriver` estava de alguma forma indisponível. 
- Não há suporte para `FirebaseJobDispatcher.ScheduleResultUnsupportedTrigger` &ndash; `Trigger`.
...
- [Defina restrições](#Setting_Constraints) &ndash; pode ser necessário apenas executar um trabalho quando determinadas condições forem atendidas. Por exemplo, somente execute um `Job` quando o dispositivo estiver carregando. 
- [Especifique quando um `Job` deve ser executado](#Setting_Job_Triggers) &ndash; o Dispatcher de trabalho firebase permite que os aplicativos especifiquem uma hora em que o trabalho deve ser executado.  
- [Declare uma estratégia de repetição para trabalhos com falha](#Setting_a_RetryStrategy) &ndash; uma _estratégia de repetição_ fornece diretrizes para o `FirebaseJobDispatcher` sobre o que fazer com `Jobs` que não são concluídos. 
...
Os parâmetros são passados para um trabalho criando um `Bundle` que é passado junto com o método `Job.Builder.SetExtras`:
...
O `Bundle` é acessado a partir da propriedade `IJobParameters.Extras` no método `OnStartJob`:
...
As restrições podem ajudar a reduzir os custos ou drenagem de bateria no dispositivo. A classe `Firebase.JobDispatcher.Constraint` define essas restrições como valores inteiros:

- `Constraint.OnUnmeteredNetwork` &ndash; só executam o trabalho quando o dispositivo está conectado a uma rede ilimitada. Isso é útil para impedir que o usuário incorra em encargos de dados.
- `Constraint.OnAnyNetwork` &ndash; executar o trabalho em qualquer rede à qual o dispositivo está conectado. Se especificado junto com `Constraint.OnUnmeteredNetwork`, esse valor terá prioridade.
- `Constraint.DeviceCharging` &ndash; executar o trabalho somente quando o dispositivo estiver carregando.

As restrições são definidas com o método `Job.Builder.SetConstraint`: 
...
O `JobTrigger` fornece diretrizes para o sistema operacional sobre quando o trabalho deve ser iniciado. Um `JobTrigger` tem uma _janela de execução_ que define um horário agendado para quando o `Job` deve ser executado. A janela de execução tem um valor de _janela inicial_ e um valor de _janela final_ . A janela inicial é o número de segundos que o dispositivo deve aguardar antes de executar o trabalho e o valor da janela final é o número máximo de segundos a aguardar antes de executar o `Job`. 

Um `JobTrigger` pode ser criado com o método `Firebase.Jobdispatcher.Trigger.ExecutionWindow`.  Por exemplo `Trigger.ExecutionWindow(15,60)` significa que o trabalho deve ser executado entre 15 e 60 segundos a partir de quando ele está agendado. O método `Job.Builder.SetTrigger` é usado para 
...
O `JobTrigger` padrão para um trabalho é representado pelo valor `Trigger.Now`, que especifica que um trabalho seja executado assim que possível depois de ser agendado.
...
- `RetryStrategy.RetryPolicyExponential` &ndash; uma política de _retirada exponencial_ aumentará o valor inicial da retirada exponencialmente após cada falha. Na primeira vez que um trabalho falhar, a biblioteca aguardará o intervalo de _initial especificado antes de reagendar o trabalho &ndash; exemplo de 30 segundos. Na segunda vez em que o trabalho falhar, a biblioteca aguardará pelo menos 60 segundos antes de tentar executar o trabalho. Após a terceira tentativa com falha, a biblioteca aguardará 120 segundos e assim por diante. O `RetryStrategy` padrão para a biblioteca do dispatcher do trabalho firebase é representado pelo objeto `RetryStrategy.DefaultExponential`. Ele tem uma retirada inicial de 30 segundos e uma retirada máxima de 3600 segundos.
- `RetryStrategy.RetryPolicyLinear` &ndash; essa estratégia é uma _retirada linear_ de que o trabalho deve ser reagendado para ser executado em intervalos definidos (até que tenha sucesso). A retirada linear é mais adequada para o trabalho que deve ser concluído assim que possível ou para problemas que serão resolvidos rapidamente. A biblioteca do dispatcher do trabalho firebase define um `RetryStrategy.DefaultLinear` que tem uma janela de reagendamento de, pelo menos, 30 segundos e até 3600 segundos.

É possível definir um `RetryStrategy` personalizado com o método `FirebaseJobDispatcher.NewRetryStrategy`. São necessários três parâmetros:

1. `int policy` &ndash; a _política_ é um dos valores `RetryStrategy`, `RetryStrategy.RetryPolicyLinear`ou `RetryStrategy.RetryPolicyExponential`anteriores.
...
3. `int maximumBackoffSeconds` &ndash; o valor _máximo de retirada_ declara o número máximo de segundos a ser atrasado antes de tentar executar o trabalho novamente. O valor padrão é 3600 segundos. 
...
É possível cancelar todos os trabalhos que foram agendados ou apenas um único trabalho usando o método `FirebaseJobDispatcher.CancelAll()` ou o método `FirebaseJobDispatcher.Cancel(string)`:
...
- `FirebaseJobDispatcher.CancelResult.NoDriverAvailable` &ndash; o `FirebaseJobDispatcher` não pode cancelar o trabalho porque não há nenhum `IDriver` válido disponível.

## <a name="summary"></a>Resumo

Este guia abordou como usar o Dispatcher de trabalhos do firebase para realizar o trabalho de forma inteligente em segundo plano. Ele abordou como encapsular o trabalho a ser executado como um `JobService` e como usar o `FirebaseJobDispatcher` para agendar esse trabalho, especificando os critérios com uma `JobTrigger` e como as falhas devem ser tratadas com um `RetryStrategy`.
