...
O namespace [Android. Renderscripts](xref:Android.Renderscripts) contém as classes para interagir com o tempo de execução do renderscript. Em particular, a [`Renderscript`](xref:Android.Renderscripts.RenderScript) classe gerenciará o ciclo de vida e os recursos do mecanismo renderscript. O aplicativo do Android deve inicializar um ou mais[`Android.Renderscripts.Allocation`](xref:Android.Renderscripts.Allocation)
...
- **tipo simples** Conceitualmente, `float` é o mesmo que um tipo de dados C ou um `char`. &ndash;

- **tipo complexo** Esse tipo é semelhante a um C `struct`. &ndash;
...
- **`ScriptC_XXXXX`** Também conhecidos como _scripts de usuário_, esses são scripts que são escritos por desenvolvedores e empacotados no apk. &ndash; No momento da compilação, o ferramentas do Android gerará classes wrapper gerenciadas que permitirão que os scripts sejam usados no aplicativo do Android.
...
- [ScriptIntrinsicBLAS](https://developer.android.com/reference/android/renderscript/ScriptIntrinsicBLAS.html) Provideshigh APIs de renderscript de desempenho para [Blas.](http://www.netlib.org/blas/) &ndash; O BLAS (subprogramas de algebra linear básico) são rotinas que fornecem blocos de construção padrão para executar operações básicas de vetor e matriz. 
...
**Criar um contexto de renderscript** &ndash; O[`Renderscript`](xref:Android.Renderscripts.RenderScript)
a classe é um wrapper gerenciado em volta do contexto renderscript e controlará a inicialização, o gerenciamento de recursos e a limpeza. O objeto renderscript é criado usando o `RenderScript.Create` método Factory, que usa um contexto do Android (como uma atividade) como um parâmetro. A linha de código a seguir demonstra como inicializar o contexto renderscript:

```csharp
Android.Renderscripts.RenderScript renderScript = RenderScript.Create(this);
```

**Criar alocações** Dependendo do script intrínseco, pode ser necessário criar um ou dois `Allocation`s. &ndash; Dos[`Android.Renderscripts.Allocation`](xref:Android.Renderscripts.Allocation)
...
Geralmente, será necessário criar um `Allocation` para manter os dados de saída de um script. Este trecho de código a seguir mostra como `Allocation.CreateTyped` usar o auxiliar para criar `Allocation` uma instância de um segundo que o mesmo tipo do original:

```csharp
Android.Renderscripts.Allocation outputAllocation = Allocation.CreateTyped(renderScript, inputAllocation.Type);
```

**Criar uma instância do wrapper de script** Cada uma das classes de wrapper de script intrínsecas deve ter métodos auxiliares (normalmente chamados `Create`) para instanciar um objeto wrapper para esse script. &ndash; O trecho de código a seguir é um exemplo de como criar `ScriptIntrinsicBlur` uma instância de um objeto Blur. O `Element.U8_4` método auxiliar criará um elemento que descreve um tipo de dados que é de quatro campos de valores inteiros de 8 bits sem sinal, adequados para manter os dados de um `Bitmap` objeto:

```csharp
Android.Renderscripts.ScriptIntrinsicBlur blurScript = ScriptIntrinsicBlur.Create(renderScript, Element.U8_4(renderScript));
```

**Atribuir alocação (ões), definir parâmetros, & executar script** A classe fornece um`ForEach` método para realmente executar o renderscript. `Script` &ndash; Esse método fará iteração em `Element` cada um `Allocation` dos dados de entrada em retenção. Em alguns casos, pode ser necessário fornecer um `Allocation` que contenha a saída.
