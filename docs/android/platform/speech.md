...
O Google fornece ao desenvolvedor um rico conjunto de APIs no namespace Android. Speech para abranger a maioria das instâncias de tornar um dispositivo "com reconhecimento de fala" (como software projetado para os cegos).  O namespace inclui a instalação para permitir que o texto seja traduzido para a fala `Android.Speech.Tts` , controle sobre o mecanismo usado para executar a tradução, bem como um número de `RecognizerIntent` s que permitem que a fala seja convertida em texto.
...
O núcleo de um dispositivo Android interpretando a fala é o uso de um `Intent` com um correspondente `OnActivityResult` .
...
Ao escutar o valor de retorno de a `StartActivityForResult` , o `OnActivityResult` método deve ser fornecido.

No exemplo a seguir, `textBox` é `TextBox` usado para gerar uma saída do que foi ditado. Ele poderia ser usado igualmente para passar o texto para alguma forma de intérprete e, a partir daí, o aplicativo pode comparar o texto e a ramificação com outra parte do aplicativo.
...
O download de um novo idioma é realizado com o uso de um `Intent` . O resultado dessa tentativa faz com que o método [OnActivityResult](xref:Android.App.Activity.OnActivityResult*) seja invocado. Ao contrário do exemplo de conversão de fala em texto (que usou o [RecognizerIntent](xref:Android.Speech.RecognizerIntent) como `PutExtra` parâmetro para o `Intent` ), os s de teste e de carregamento `Intent` são `Action` baseados:

- [TextToSpeech. Engine. ActionCheckTtsData](xref:Android.Speech.Tts.TextToSpeech.Engine.ActionCheckTtsData) &ndash; inicia uma atividade do mecanismo de plataforma `TextToSpeech` para verificar a instalação e a disponibilidade corretas dos recursos de idioma no dispositivo.
...
Para que uma atividade possa converter o texto em fala, o método de interface `OnInit` deve ser implementado (esse é o segundo parâmetro especificado para a instanciação da `TextToSpeech` classe). Isso inicializa o ouvinte e testa o resultado.
...
- [Conversão de Texto em Fala (exemplo)](/samples/xamarin/monodroid-samples/platformfeatures-texttospeech)
- [Fala para texto (exemplo)](/samples/xamarin/monodroid-samples/platformfeatures-speechtotext)
