...
Ao trabalhar com C# e .NET em um aplicativo Xamarin. Mac, você tem acesso aos mesmos bancos de dados SQLite que um aplicativo Xamarin. iOS ou Xamarin. Android pode acessar.
...
Para dados do SQLite que serão associados a elementos da interface do usuário no Interface Builder do Xcode, é altamente recomendável que você acesse o banco de dado do SQLite diretamente (em oposição ao uso de uma técnica, como um ORM), pois você tem controle total sobre a forma como os dados são gravados e lidos no Database.

Como vimos na ligação de dados e na documentação de [codificação de valor chave](~/mac/app-fundamentals/databinding.md) , usando a codificação de valor-chave e técnicas de vinculação de dados em seu aplicativo Xamarin. Mac, você pode diminuir muito a quantidade de código que precisa escrever e manter para preencher e trabalhar com elementos da interface do usuário. Quando combinado com acesso direto a um banco de dados SQLite, ele também pode reduzir consideravelmente a quantidade de código necessária para ler e gravar dados nesse banco.
...
Todas as alterações feitas nas propriedades **Name**, **profissão** ou **ismanager** serão enviadas para o banco de dados se eles tiverem sido salvos lá antes (por exemplo, se a `_conn` variável não for `null` ). Em seguida, vamos examinar os métodos que adicionamos para **criar**, **Atualizar**, **carregar** e **excluir** pessoas do banco de dados.
...
Posteriormente, definimos os valores para os parâmetros usando o `Parameters.AddWithValue` método no `SQLiteCommand` . Usando parâmetros, garantimos que os valores (como uma aspa simples) sejam codificados corretamente antes de serem enviados ao SQLite. Exemplo:
...
Como **criar** acima, obtemos um `SQLiteCommand` dos aprovados `SQLiteConnection` e definimos nosso SQL para atualizar nosso registro (fornecendo parâmetros):
...
Por fim, usamos um leitor de dados para executar a consulta e retornar os campos de registro (que copiamos para a instância da `PersonModel` classe):
...
Ele é chamado sempre que o `TableName` `IDField` valor de `DisplayField` Propriedades ou é alterado.
...
O `ItemCount` retorna o número de itens pré-calculados na lista conforme calculado quando as `TableName` `IDField` `DisplayField` Propriedades ou são alteradas:
...
Observe que estamos usando as `LIMIT` instruções e `OFFSET` em nosso comando do SQLite para limitar a um registro que é necessário.
...
Se o valor não puder ser encontrado, o `NSRange.NotFound` valor será retornado e todos os itens serão desmarcados na lista suspensa.
...
Na `AwakeFromNib` substituição do método, primeiro anexamos uma instância da nossa fonte de dados da caixa de combinação personalizada:
...
Vamos adicionar uma nova classe ao projeto e chamá-la `OccupationModel` . Em seguida, vamos editar o arquivo **OccupationModel.cs** e torná-lo semelhante ao seguinte:
...
Como exemplo de uso, adicionaremos uma exibição de tabela à nossa interface do usuário no construtor de interface do Xcode. Vamos expor essa exibição de tabela por meio de uma tomada ( `OccupationTable` ) para que possamos acessá-la por meio de código C#:
...
Quando criamos uma instância dessa classe mais tarde, passaremos nossa conexão de banco de dados Open SQLite.NET. O `LoadOccupations` método consulta o banco de dados e copia os registros encontrados na memória (usando nosso `OccupationModel` modelo de dado).
...
Aqui, usamos a coleção da fonte de dados `Occupations` (que carregamos a partir do SQLite.net Database) para preencher as colunas da nossa tabela por meio da `GetViewForItem` substituição do método.
