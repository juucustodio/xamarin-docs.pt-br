...
No Xamarin. Mac, um aplicativo se une a dois mundos: há o tempo de execução baseado em Objective-C contendo instâncias de classes nativas (`NSString`, `NSApplication`, C# etc.) e há o tempo de execução que contém instâncias de classes gerenciadas (`System.String`,`HttpClient`, etc.) . Entre esses dois mundos, o Xamarin. Mac cria uma ponte de duas vias para que um aplicativo possa chamar métodos (seletores) em Objective-C (como `NSApplication.Init`) e Objective-C possa chamar C# os métodos do aplicativo de volta (como métodos em um delegado de aplicativo). Em geral, as chamadas para Objective-C são manipuladas de forma transparente por meio de **P/Invokes** e um código de tempo de execução que o Xamarin oferece.

<a name="exposing-classes" />

## <a name="exposing-c-classes--methods-to-objective-c"></a>Expondo C# classes/métodos para Objective-C

No entanto, para o Objective-C chamar de volta C# para os objetos de um aplicativo, eles precisam ser expostos de uma forma que o Objective-c possa entender. Isso é feito por meio dos atributos `Register` e `Export`. Veja o exemplo a seguir:
...
Na maioria dos casos, esse é um detalhe de implementação que o desenvolvedor pode ignorar, pois a maioria dos retornos de chamada que um aplicativo recebe será por meio de métodos substituídos em classes `base` (como `AppDelegate`, `Delegates`, `DataSources`) ou em **ações** passadas para APIs. Em todos esses casos, `Export` atributos não são necessários no C# código.
...
- **Melhores logs de falhas "nativos"** – se um aplicativo Xamarin. Mac falhar no código nativo, que é ocorrência comum ao fazer chamadas inválidas em APIs Cocoa (como enviar um `null` em um método que não o aceita), os logs de falha nativos com quadros JIT são difícil de analisar. Como os quadros JIT não têm informações de depuração, haverá várias linhas com deslocamentos hexadecimais e nenhuma pista sobre o que estava acontecendo. A AOT gera quadros nomeados "reais" e os rastreamentos são muito mais fáceis de ler. Isso também significa que o aplicativo Xamarin. Mac irá interagir melhor com ferramentas nativas como **lldb** e **instrumentos**.
...
A AOT está habilitada no Xamarin. Mac clicando duas vezes **no nome do projeto** no **Gerenciador de soluções**, navegando até o **Mac Build** e adicionando `--aot:[options]` ao campo **MMP arguments adicionais:** (em que `[options]` é uma ou mais opções para controle o tipo de AOT, veja abaixo). Por exemplo:
...
- `core`-AOT compila os assemblies `Xamarin.Mac`, `System` e `mscorlib`.
- `sdk`-AOT compila o `Xamarin.Mac` e os assemblies da BCL (biblioteca de classes base).
...
Por exemplo, `--aot:all,-MyAssembly.dll` habilitaria a compilação AOT em todos os assemblies no grupo de monopacote _, exceto_ `MyAssembly.dll` e `--aot:core|hybrid,+MyOtherAssembly.dll,-mscorlib.dll` habilitaria o híbrido, o código AOT incluiria o `MyOtherAssembly.dll` e excluindo o `mscorlib.dll`.
...
Nos bastidores de qualquer aplicativo Xamarin. Mac está a estrutura Cocoa da Apple e do tempo de execução do Objective-C. Criar uma ponte entre esse "mundo nativo" e o "mundo gerenciado" do C# é a principal responsabilidade do Xamarin. Mac. Parte dessa tarefa é tratada pelo registrador, que é executado dentro do método `NSApplication.Init ()`. Essa é uma razão pela qual qualquer uso de APIs Cocoa no Xamarin. Mac exige que `NSApplication.Init` seja chamado primeiro.
...
O registro estático parcial, introduzido pela primeira vez no Xamarin. iOS 8,13, oferece ao desenvolvedor o melhor das duas opções. Ao pré-testar previamente as informações de registro de cada elemento em `Xamarin.Mac.dll` e enviar essas informações com o Xamarin. Mac em uma biblioteca estática (que só precisa ser vinculada no momento da compilação), a Microsoft removeu a maior parte do tempo de reflexão do registrador dinâmico Embora não afete o tempo de compilação.

### <a name="enabling-the-partial-static-registrar"></a>Habilitando o registrador estático parcial

O registrador de estática parcial está habilitado no Xamarin. Mac clicando duas vezes **no nome do projeto** no **Gerenciador de soluções**, navegando até o **Mac Build** e adicionando `--registrar:static` ao campo **MMP Arguments adicionais:** . Por exemplo:
