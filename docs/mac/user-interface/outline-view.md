...
[![](outline-view-images/populate03.png "Uma execução de aplicativo de exemplo")](outline-view-images/populate03.png#lightbox)
...
[![](outline-view-images/edit03.png "Selecionando um modo de exibição de estrutura de tópicos da biblioteca")](outline-view-images/edit03.png#lightbox)
...
[![](outline-view-images/edit04.png "Editando as restrições")](outline-view-images/edit04.png#lightbox)
...
> [!IMPORTANT]
> A menos que você esteja mantendo um aplicativo Xamarin. Mac `NSView` herdado, exibições de estrutura `NSCell` de tópicos baseadas devem ser usadas em exibições de tabela baseadas em. `NSCell`é considerado herdado e pode não ter suporte no futuro.
...
[![](outline-view-images/edit11.png "Selecionando a exibição de célula da tabela")](outline-view-images/edit10.png#lightbox)
...
Essa classe tem armazenamento para os itens de nossa exibição de estrutura de `GetChildrenCount` tópicos e substitui o para retornar o número de linhas na tabela. O `GetChild` retorna um item pai ou filho específico (conforme solicitado pelo modo de exibição de estrutura de `ItemExpandable` tópicos) e define o item especificado como um pai ou filho.
...
Edite `ProductOutlineDelegate.cs` o arquivo e faça com que ele se pareça com o seguinte:
...
Quando criamos uma instância do `ProductOutlineDelegate`, também passamos uma instância `ProductOutlineDataSource` do que fornece os dados para o contorno. O `GetView` método é responsável por retornar uma exibição (dados) para exibir a célula de uma coluna e uma linha. Se possível, uma exibição existente será reutilizada para exibir a célula, se não for necessário criar uma nova exibição.

Para preencher o contorno, vamos editar o `MainWindow.cs` arquivo e fazer com `AwakeFromNib` que o método fique semelhante ao seguinte:
...
[![](outline-view-images/populate02.png "A exibição recolhida")](outline-view-images/populate02.png#lightbox)

Se expandirmos um nó no modo de exibição de estrutura de tópicos, ele será semelhante ao seguinte:

[![](outline-view-images/populate03.png "A exibição expandida")](outline-view-images/populate03.png#lightbox)
...
Isso permitirá que o usuário selecione uma única linha no modo de exibição de estrutura de tópicos. Retorne `false` para `ShouldSelectItem` o para qualquer item que você não queira que o usuário possa selecionar ou `false` para cada item se não quiser que o usuário seja capaz de selecionar qualquer item.
...
Isso permitirá que o usuário selecione uma única linha no modo de exibição de estrutura de tópicos. Retorne `false` para `ShouldSelectRow` o para qualquer item que você não queira que o usuário possa selecionar ou `false` para cada item se não quiser que o usuário seja capaz de selecionar qualquer item.

<a name="Type_to_Select_Row" />

## <a name="type-to-select-row"></a>Digite para selecionar a linha

Se você quiser permitir que o usuário digite um caractere com o modo de exibição de estrutura de tópicos selecionado e selecione a primeira linha que tem esse caractere, `Main.storyboard` clique duas vezes no arquivo para abri-lo para edição no interface Builder. Selecione o modo de exibição de estrutura de tópicos na **hierarquia de interface** e marque a caixa de seleção **tipo selecionar** no **Inspetor de atributo**:

[![](outline-view-images/type01.png "Editando o tipo de linha")](outline-view-images/type01.png#lightbox)
...
O `GetNextTypeSelectMatch` método usa o dado `searchString` e retorna o item do primeiro `Product` que tem essa cadeia de `Title`caracteres.
...
Se você quiser permitir que o usuário edite os valores de uma determinada célula, edite `ProductOutlineDelegate.cs` o arquivo e altere `GetViewForItem` o método da seguinte maneira:
