...
Extraia o arquivo zip para a pasta `/Applications` em seu servidor de Build e inicie-o da mesma forma que qualquer outro aplicativo do OS X.
...
Clique nesse botão e preencha o **nome** e o **caminho** para os campos do **MSBuild** no formulário que aparece. O nome da sua instalação do **MSBuild** deve ser algo significativo, enquanto o **caminho para o MSBuild** deve ser o caminho para `xbuild`, que normalmente é **/library/frameworks/mono.Framework/Commands/xbuild**. Depois de salvar as alterações clicando no botão salvar ou aplicar na parte inferior da página, o Jenkins poderá usar `xbuild` para compilar suas soluções.
...
3. Para confirmar se o Team Explorer Everywhere está instalado, abra uma sessão de terminal e execute o comando `tf`. Se o tf estiver configurado corretamente, você verá a seguinte saída em sua sessão de terminal:
...
Depois que o cliente de linha de comando para TFS for instalado, Jenkins deverá ser configurado com o caminho completo para o cliente de linha de comando `tf`. Role para baixo a página **Configurar sistema [Jenkins]** até encontrar a seção Team Foundation Server, conforme mostrado na seguinte captura de tela:
...
Insira o caminho completo para o comando `tf` e clique no botão **salvar** .
...
3. Altere o valor do elemento `<usesecurity></usesecurity>` de `true` para `false`.
4. Exclua o `<authorizationstrategy></authorizationstrategy>` e os elementos de `<securityrealm></securityrealm>` do arquivo.
...
|INPUT_APK|O APK que é criado por `xbuild`.|
|SIGNED_APK|O APK assinado produzido por `jarsigner`.|
|FINAL_APK|Esse é o APK alinhado em zip que é produzido por `zipalign`.|
...
Observe que algumas das variáveis de ambiente para os arquivos APK são criadas na variável de ambiente `WORKSPACE`.
...
A captura de tela a seguir mostra um exemplo de como inserir os comandos `jarsigner` e `zipalign` na etapa:
