...
Objective-C e C# são linguagens muito diferentes do ponto de vista sintático e de runtime. Objective-C é uma linguagem dinâmica e usa um esquema de passagem de mensagem, enquanto C# é estaticamente tipado. A sintaxe, Objective-C, é como Smalltalk, enquanto o C# deriva grande parte de sua sintaxe fundamental do Java, embora tenha se esdurado para incluir muitos recursos além do Java nos últimos anos.
...
Em seguida, quando uma instância de `UITextView` for criada no código, o método estará disponível na lista de preenchimento automático, conforme mostrado abaixo:
 ...
 ![O método disponível no preenchimento automático](primer-images/01-extensionmethodintellisense.png)
...
Para obter mais informações sobre expressões lambda, confira o [Guia de Programação em C#](/dotnet/csharp/language-reference/operators/lambda-expressions) da Microsoft.
