...
- O `OpenURL` método da `UIApplication` classe é chamado de forma assíncrona e agora dá suporte a um manipulador de conclusão que é chamado depois que a ação abrir é concluída.
