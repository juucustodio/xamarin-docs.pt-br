...
A API para mostrar um produto em qualquer aplicativo é simples: requer apenas que você crie e exiba uma `SKStoreProductViewController`. Siga estas etapas para criar e mostrar um produto:

1. Crie um objeto `StoreProductParameters` para passar parâmetros para o controlador de exibição, incluindo o `productId` no construtor.
1. Crie uma instância do `SKProductViewController`. Atribua-o a um campo de nível de classe.
1. Atribua um manipulador ao evento de `Finished` do controlador de exibição, que deve ignorar o controlador de exibição. Esse evento é chamado quando o usuário pressiona cancelar; ou, de outra forma, finaliza uma transação dentro do controlador de exibição.
1. Chame o método `LoadProduct` passando o `StoreProductParameters` e um manipulador de conclusão. O manipulador de conclusão deve verificar se a solicitação do produto foi bem-sucedida e, nesse caso, apresentar a `SKProductViewController` modalmente. O tratamento de erros apropriado deve ser adicionado caso o produto não possa ser recuperado.

### <a name="example"></a>Exemplo

O projeto *ProductView* no código de exemplo *StoreKit* para este artigo implementa um método `Buy` que aceita a ID da Apple de qualquer produto e exibe o `SKStoreProductViewController`. O código a seguir exibe as informações do produto para qualquer ID da Apple determinada:
...
O aplicativo de exemplo inclui código que mostra como abrir a loja de aplicativos, o iTunes ou o eróticos da iBookstore em versões anteriores do iOS. Use o método `OpenUrl` para abrir uma URL **iTunes.com** corretamente criada.
...
Em qualquer link de produto do iTunes (para aplicativos, música ou livros), localize a parte da URL que começa com `id` e use o número a seguir.
...
Depois de carregar um pacote de conteúdo, ele pode ser substituído por conteúdo mais recente. O novo conteúdo deve ser carregado e enviado para revisão/aprovação por meio do processo normal. Aumente o campo `ContentVersion` em pacotes de conteúdo atualizados para indicar que ele é mais recente.
...
Observe a estrutura do diretório do pacote (especialmente o local dos arquivos no subdiretório `Contents`), pois você precisará entender essas informações para extrair os arquivos do pacote no dispositivo.
...
- **SKPaymentTransaction** – nova propriedade adicionada: `Downloads`, que contém uma coleção de objetos `SKDownload` se este produto tiver conteúdo hospedado disponível para download.
...
Detalhes da nova classe de `SKDownload`:
...
- **ContentURL** – local do arquivo em que o conteúdo foi colocado no disco, no diretório `Cache`. Preenchido apenas depois que o download for concluído.
...
Altere a substituição de `UpdatedTransactions` existente para verificar se há conteúdo para download e chame `StartDownloads` se necessário:
...
O novo método substituído `UpdatedDownloads` é mostrado abaixo. O kit de loja chama esse método depois que `StartDownloads` é disparado no `UpdatedTransactions`. Esse método é chamado *várias vezes* em intervalos indeterminados para fornecer o progresso do download e, em seguida, novamente quando o download for concluído. Observe que o método aceita uma matriz de objetos `SKDownload`, de modo que cada chamada de método pode fornecer o status de vários downloads na fila. Conforme mostrado na implementação abaixo, os status de download são verificados toda vez e a ação apropriada é executada.
...
O conteúdo hospedado foi baixado com êxito e descompactado no diretório `Cache`. A estrutura do. O arquivo PKG requer que todos os arquivos sejam salvos em um subdiretório `Contents`, portanto, o código a seguir extrai arquivos de dentro do subdiretório `Contents`.

O código itera em todos os arquivos no pacote de conteúdo e os copia para o diretório `Documents`, em uma subpasta chamada para o `ProductIdentifier`. Finalmente, ele chama `CompleteTransaction`, que chama `FinishTransaction` para remover a transação da fila de pagamentos.
...
Quando `FinishTransaction` é chamado, os arquivos baixados não são mais garantidos no diretório `Cache`. Todos os arquivos devem ser copiados antes de chamar `FinishTransaction`.
...
Uma alternativa pode ser copiar o conteúdo em uma pasta chamada para a versão e controlar qual é a versão atual (por exemplo, no `NSUserDefaults` ou onde você armazenar os registros de compra concluídos).
...
Se o código chamar `FinishTransaction` na fila de pagamentos antes de o download ser `Finished`, esse download será cancelado automaticamente.
