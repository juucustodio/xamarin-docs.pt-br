...
No núcleo do Xamarin. iOS, há um mecanismo de interoperabilidade que une o mundo do C# com o mundo Objective-C, bem como associações para as APIs baseadas em iOS C, como CoreGraphics e [OpenGL ES](#opengles).
...
- Incentive a exploração no IDE das APIs:
  ...
  - Por exemplo, em vez de expor uma matriz de tipo fraco como esta:
    ...
    Isso dá Visual Studio para Mac a capacidade de fazer a conclusão automática ao navegar pela API, disponibilizar todas as `System.Array` operações no valor retornado e permitir que o valor de retorno participe no LINQ.
...
- Suporte ao padrão delegado Objective-C:
  ...
  - Expor delegados do C# (lambdas, métodos anônimos e `System.Delegate` ) para APIs Objective-C como blocos
...
O namespace [ObjCRuntime](xref:ObjCRuntime) permite aos desenvolvedores preencher os mundos entre C# e Objective-C.
...
O Xamarin. iOS é espelhado em C# a hierarquia de classes de Objective-C. Por exemplo, a classe base Objective-C NSObject é utilizável do C# por meio de [Foundation. NSObject](xref:Foundation.NSObject).
...
A partir do Xamarin. iOS 8,10, é possível criar categorias Objective-C a partir do C#.

Isso é feito usando o `Category` atributo, especificando o tipo a ser estendido como um argumento para o atributo. O exemplo a seguir fará com que a instância estenda o NSString.

```csharp
[Category (typeof (NSString))]
```

Cada método de categoria está usando o mecanismo normal para exportar métodos para Objective-C usando o `Export` atributo:
...
Um cenário em que isso é útil é adicionar um método a um conjunto inteiro de classes em sua base de código, por exemplo, fazer `UIViewController` com que todas as instâncias relatem que podem girar:
...
O namespace [UIKit](xref:UIKit) contém um mapeamento de um para um para todos os componentes da interface do usuário que compõem Cocoatouch na forma de classes C#. A API foi modificada para seguir as convenções usadas na linguagem C#.

Os delegados do C# são fornecidos para operações comuns. Consulte a seção [delegados](#delegates) para obter mais informações.
...
O Xamarin. iOS não é meramente uma associação à plataforma Objective-C subjacente. Ele estende o sistema de tipos .NET e o sistema de expedição para melhor misturar C# e Objective-C.

Assim como P/Invoke é uma ferramenta útil para invocar bibliotecas nativas no Windows e no Linux, ou como o suporte a IJW pode ser usado para interoperabilidade COM no Windows, o Xamarin. iOS estende o tempo de execução para dar suporte à vinculação de objetos C# aos objetos Objective-C.
...
Onde fazia sentido, os tipos C# são expostos em vez de tipos de base de nível baixo, para o universo em C#.  Isso significa que [a API usa o tipo de "cadeia de caracteres" c# em vez de NSString](~/ios/internals/api-design/nsstring.md) e usa matrizes c# com rigidez de tipos em vez de expor NSArray.

Em geral, no design do Xamarin. iOS e Xamarin. Mac, o `NSArray` objeto subjacente não é exposto. Em vez disso, o tempo de execução converte automaticamente as `NSArray` s em matrizes fortemente tipadas de alguma `NSObject` classe. Portanto, o Xamarin. iOS não expõe um método de tipo fraco, como GetViews, para retornar um NSArray:
...
Há alguns métodos expostos no `NSArray` , para os casos de canto em que você pode querer usar um `NSArray` diretamente, mas seu uso é desencorajado na associação de API.

Além disso, na **API clássica** em vez de expor `CGRect` , `CGPoint` e `CGSize` da API CoreGraphics, substituímos aquelas pelas `System.Drawing` implementações `RectangleF` , `PointF` e `SizeF` como eles ajudarão os desenvolvedores a preservar o código OpenGL existente que usa OpenTK. Ao usar o novo **API unificada**de 64 bits, a API CoreGraphics deve ser usada.

#### <a name="inheritance"></a>Herança

O design da API do Xamarin. iOS permite aos desenvolvedores estender os tipos de Objective-C nativos da mesma forma que eles estendem um tipo C#, usando a palavra-chave "override" em uma classe derivada, bem como encadeando a implementação de base usando a palavra-chave C# "base".
...
Ao criar classes do .NET que são instâncias de tipos criadas por Interface Builder, você precisa fornecer um construtor que aceite um único `IntPtr` parâmetro.
...
Objective-C e C# têm significados diferentes para o delegado de palavra em cada idioma.

No mundo Objective-C e na documentação que você encontrará online sobre CocoaTouch, um delegado normalmente é uma instância de uma classe que responderá a um conjunto de métodos. Isso é muito semelhante a uma interface C#, com a diferença de que os métodos nem sempre são obrigatórios.
...
2. [Fortemente tipado por meio de uma `Delegate` Propriedade](#strongly-typed-via-a-delegate-property)
3. [Com rigidez de tipos por meio de uma `WeakDelegate` Propriedade](#loosely-typed-via-the-weakdelegate-property)
...
Por exemplo, você pode usar esse mecanismo para descartar o teclado na tela no manipulador de um `UITextField` :
...
Esse padrão também é usado para controlar o comportamento de determinados controles, por exemplo, no caso UIWebView, a propriedade [UIWebView. ShouldStartLoad](xref:UIKit.UIWebView.ShouldStartLoad) permite que a `UIWebView` instância controle se o `UIWebView` carregará ou não uma página.
...
Em qualquer lugar em que uma propriedade fortemente tipada `Delegate` é exposta na associação do Xamarin. Ios, uma `WeakDelegate` propriedade correspondente também é exposta.
...
Observe que depois que a `WeakDelegate` propriedade tiver sido atribuída, a `Delegate` propriedade não será usada. Além disso, se você implementar o método em uma classe base herdada que deseja [exportar], deverá torná-lo um método público.
...
Isso instrui o idioma a criar e construir uma instância da classe "SomethingDelegate" e atribuir o valor à propriedade delegate na variável foo. Esse mecanismo tem suporte do Xamarin. iOS e do C# a sintaxe é:
...
O segundo formulário é fornecer notificação para eventos. Nesses casos, embora ainda exponhamos a API na forma descrita acima, também fornecemos eventos C#, que devem ser mais simples de usar para operações rápidas e integradas com delegados anônimos e expressões lambda em C#.
...
As duas opções estão disponíveis onde fazem sentido, mas como um programador, você deve escolher uma ou outra. Se você criar sua própria instância de um Respondente/delegado com rigidez de tipos e atribuí-la, os eventos do C# não serão funcionais. Se você usar os eventos C#, os métodos em sua classe Respondente/delegado nunca serão chamados.

O exemplo anterior que `UIWebView` foi usado pode ser escrito usando lambdas C# 3,0 como este:
...
Conforme detalhado anteriormente, o Xamarin. iOS dá suporte ao modelo de programação baseado em evento do C# e ao padrão delegado do Objective-C, no qual você pode criar uma nova classe que implementa o delegado e substitui os métodos desejados.
...
Por exemplo, se você quisesse que sua classe respondesse à `UITextFieldDelegate.textFieldShouldClear` : Message e a `UIWebViewDelegate.webViewDidStartLoad` : na mesma instância de uma classe, precisaria usar a declaração de atributo [Export]:
...
Os nomes C# dos métodos não são importantes; Tudo o que importa são as cadeias de caracteres passadas para o atributo [Export].

Ao usar esse estilo de programação, verifique se os parâmetros do C# correspondem aos tipos reais que o mecanismo de tempo de execução passará.
...
O nome do seletor Objective-C ("applicationDidFinishLaunching:") é declarado com o atributo de exportação e a classe é registrada com o `[Register]` atributo.
...
As vantagens são que não há necessidade de se aprofundar nos arquivos de cabeçalho Objective-C para localizar o seletor, os tipos de argumentos ou o mapeamento para C#, e que você obtém o IntelliSense de Visual Studio para Mac, juntamente com tipos fortes
...
>
> [![Inserindo um nome de item no designer do iOS](images/designeroutlet.png)](images/designeroutlet.png#lightbox)
>
...
Esta é uma descrição de baixo nível de como a informações se integra ao C# e é fornecida para usuários avançados do Xamarin. iOS. Ao usar Visual Studio para Mac o mapeamento é feito automaticamente em segundo plano usando o código gerado no vôo para você.
...
Isso carrega a interface do usuário do NIB. Agora, para acessar as saídas, é necessário informar ao tempo de execução que queremos acessá-las. Para fazer isso, a `UIViewController` subclasse precisa declarar as propriedades e anotá-las com o atributo [Connect]. Dessa forma:
...
Criar novos seletores em C# é muito fácil: basta criar uma nova instância da `ObjCRuntime.Selector` classe e usar o resultado em qualquer lugar na API que o exija. Por exemplo:

```csharp
var selector_add = new Selector ("add:plus:");
```

Para um método C# responder a uma chamada de seletor, ele deve herdar do `NSObject` tipo e o método C# deve ser decorado com o nome do seletor usando o `[Export]` atributo. Por exemplo:
...
A maioria das classes no Xamarin. iOS que derivam do `NSObject` irá expor construtores específicos à funcionalidade do objeto, mas eles também expõem vários construtores que não são imediatamente óbvios.
...
Esse é o construtor padrão para uma classe e nas classes fornecidas pelo Xamarin. iOS, isso Inicializa a classe Foundation. NSObject e todas as classes entre e no final, encadeia isso para o método Objective-C `init` na classe.

```csharp
public Foo (NSObjectFlag x)
```

Esse construtor é usado para inicializar a instância, mas impede que o código chame o método Objective-C "init" no final. Normalmente, você usa isso quando você já se registrou para inicialização (quando você usa `[Export]` em seu construtor) ou quando você já fez sua inicialização por outra média.
...
O Xamarin. iOS tem um coletor de lixo que cuidará da liberação de recursos para você quando eles não estiverem mais em uso. Além do coletor de lixo, todos os objetos que derivam de `NSObject` implementam a `System.IDisposable` interface.
...
Mas descartar um objeto em C# não significa que o objeto será necessariamente destruído. Tudo o que você faz é liberar a referência que o C# tinha ao objeto. É possível que o ambiente Cocoa tenha mantido uma referência para seu próprio uso. Por exemplo, se você definir a propriedade Image de UIImageView como uma imagem e, em seguida, descartar a imagem, o UIImageView subjacente terá obtido sua própria referência e manterá uma referência a esse objeto até que ele seja concluído usando-o.
...
Internamente, quando o mono cria [referências NSString de cadeias de caracteres C#](~/ios/internals/api-design/nsstring.md), ele as descartará imediatamente para reduzir a quantidade de trabalho que o coletor de lixo precisa fazer. Quanto menos objetos forem abordados, mais rápido será a execução do GC.
