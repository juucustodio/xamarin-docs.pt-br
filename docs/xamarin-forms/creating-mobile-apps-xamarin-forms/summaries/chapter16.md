...
> [!NOTE]
> O `BindingMode` enumeração agora também inclui `OnTime` para a aplicação de uma associação somente quando o contexto de associação é alterado e não quando a propriedade de origem é alterada.
