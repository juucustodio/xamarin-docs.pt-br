...
ms.locfileid: "37935635"
---
# <a name="summary-of-chapter-26-custom-layouts"></a>Resumo do Capítulo 26. Layouts personalizados

Xamarin. Forms inclui várias classes derivadas [ `Layout<View>` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Layout%3CT%3E/):
...
Layout começa na parte superior da árvore visual com a página e, em seguida, continua por meio de todas as ramificações. É o método público mais importante no layout [ `Layout` ](https://developer.xamarin.com/api/member/Xamarin.Forms.VisualElement.Layout/p/Xamarin.Forms.Rectangle/) definidos por `VisualElement`. Cada elemento que é pai de outras chamadas de elementos `Layout` para cada um dos seus filhos para dar o filho de um tamanho e posição em relação ao próprio na forma de um [ `Rectangle` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Rectangle/) valor. Eles `Layout` chamadas propagam através da árvore visual.
...
- [`UpdateChildrenLayout`](https://developer.xamarin.com/api/member/Xamarin.Forms.Page.UpdateChildrenLayout()/) para `Page` derivativos e [ `UpdateChildrenLayout` ](https://developer.xamarin.com/api/member/Xamarin.Forms.Layout.UpdateChildrenLayout()/) para `Layout` derivativos, que chama
- [`LayoutChildren`](https://developer.xamarin.com/api/member/Xamarin.Forms.Page.LayoutChildren/p/System.Double/System.Double/System.Double/System.Double/) para `Page` derivativos e [ `LayoutChildren` ](https://developer.xamarin.com/api/member/Xamarin.Forms.Layout.LayoutChildren/p/System.Double/System.Double/System.Double/System.Double/) para `Layout` derivados.
...
- [`LayoutChanged`](https://developer.xamarin.com/api/event/Xamarin.Forms.Page.LayoutChanged/) para `Page` derivativos e [ `LayoutChanged` ](https://developer.xamarin.com/api/event/Xamarin.Forms.Layout.LayoutChanged/) para `Layout` derivados
...
O `Measure` método acomoda a [ `Margin` ](https://developer.xamarin.com/api/property/Xamarin.Forms.View.Margin/) propriedade e inclui um argumento do tipo [ `MeasureFlag` ](https://developer.xamarin.com/api/type/Xamarin.Forms.MeasureFlags/), que tem dois membros:
...
Ambos `GetSizeRequest`e `Measure` retornam um valor do tipo [ `SizeRequest` ](https://developer.xamarin.com/api/type/Xamarin.Forms.SizeRequest/), que tem duas propriedades:
...
- [`WidthRequest`](https://developer.xamarin.com/api/property/Xamarin.Forms.VisualElement.WidthRequest/)do tipo `double`, afeta a `Request` propriedade de `SizeRequest`
- [`HeightRequest`](https://developer.xamarin.com/api/property/Xamarin.Forms.VisualElement.HeightRequest/) do tipo `double`, afeta a `Request` propriedade de `SizeRequest`
- [`MinimumWidthRequest`](https://developer.xamarin.com/api/property/Xamarin.Forms.VisualElement.MinimumWidthRequest/) do tipo `double`, afeta a `Minimum` propriedade de `SizeRequest`
- [`MinimumHeightRequest`](https://developer.xamarin.com/api/property/Xamarin.Forms.VisualElement.MinimumHeightRequest/) do tipo `double`, afeta a `Minimum` propriedade de `SizeRequest`
...
O `Layout` classe define um método protegido semelhante denominado [ `InvalidateLayout` ](https://developer.xamarin.com/api/member/Xamarin.Forms.Layout.InvalidateLayout()/), que um `Layout` derivado deve chamar qualquer alteração que afeta como ele posiciona e dimensiona seus filhos.
...
2. Um `Layout<T>` derivativo que define as propriedades vinculáveis anexadas deve substituir [ `OnAdded` ](https://developer.xamarin.com/api/member/Xamarin.Forms.Layout%3CT%3E.OnAdded/p/T/) para adicionar um manipulador de propriedade alterada para seus filhos e [ `OnRemoved` ](https://developer.xamarin.com/api/member/Xamarin.Forms.Layout%3CT%3E.OnRemoved/p/T/) para remover os manipulador. O manipulador deve verificar para alterações nesses anexados propriedades vinculáveis e responder ao chamar `InvalidateLayout`.

3. Um `Layout<T>` derivativo que implementa um cache de tamanhos de filho deve substituir `InvalidateLayout` e [ `OnChildMeasureInvalidated` ](https://developer.xamarin.com/api/member/Xamarin.Forms.Layout.OnChildMeasureInvalidated()/) e limpar o cache quando esses métodos são chamados.
