...
Uma folha de estilos pode ser carregada e analisada com [`StyleSheet`](xref:Xamarin.Forms.StyleSheets.StyleSheet) a classe antes de ser adicionada [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)a:
...
Como alternativa, uma folha de estilos pode ser carregada e analisada com [`StyleSheet`](xref:Xamarin.Forms.StyleSheets.StyleSheet) a classe, antes de ser adicionada [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)a uma, inserindo-a `CDATA` em uma seção:
...
Os elementos na árvore visual podem ser selecionados por tipo com o seletor de `element` não diferenciação de maiúsculas e minúsculas:
...
Esse seletor identifica [`StackLayout`](xref:Xamarin.Forms.StackLayout) quaisquer elementos em páginas que consomem a folha de estilos e define suas margens com uma espessura uniforme de 20.
...
Esse seletor identifica [`ContentPage`](xref:Xamarin.Forms.ContentPage) todos os elementos que consomem a folha de estilos e define `lightgray`a cor do plano de fundo como.
...
Elementos individuais na árvore visual podem ser selecionados com o `#id` seletor de maiúsculas e minúsculas:
...
Este seletor identifica o elemento [`StyleId`](xref:Xamarin.Forms.Element.StyleId) cuja propriedade está definida `listView`como. No entanto, `StyleId` se a propriedade não for definida, o seletor voltará a `x:Name` usar o do elemento. Portanto, no exemplo de XAML a seguir, `#listView` o [`ListView`](xref:Xamarin.Forms.ListView) seletor identificará `x:Name` o cujo atributo está `listView`definido como e definirá a cor do plano `lightgray`de fundo como.
...
Elementos com um atributo de classe específico podem ser selecionados com o `.class` seletor de maiúsculas e minúsculas:
...
Elementos filho na árvore visual podem ser selecionados com o seletor de não `element element` diferenciação de maiúsculas e minúsculas:
...
Esse seletor identifica [`Image`](xref:Xamarin.Forms.Image) quaisquer elementos que sejam filhos [`ListView`](xref:Xamarin.Forms.ListView) de elementos e define sua altura e largura como 60. Portanto, no exemplo de XAML a seguir, `listview image` o seletor identificará o [`Image`](xref:Xamarin.Forms.Image) que é filho de [`ListView`](xref:Xamarin.Forms.ListView)e definirá sua altura e largura como 60.
...
Esse seletor identifica [`Image`](xref:Xamarin.Forms.Image) todos os elementos que são filhos [`StackLayout`](xref:Xamarin.Forms.StackLayout) diretos de elementos e define sua altura e largura como 200. Portanto, no exemplo de XAML a seguir, `stacklayout>image` o seletor identificará o [`Image`](xref:Xamarin.Forms.Image) que é um filho direto [`StackLayout`](xref:Xamarin.Forms.StackLayout)do e definirá sua altura e largura como 200.
...
|---|---|---|
|`.class`|`.header`|Seleciona todos os elementos com `StyleClass` a propriedade que contém ' header '. Observe que esse seletor diferencia maiúsculas de minúsculas.|
|`#id`|`#email`|Seleciona todos os elementos `StyleId` com definido `email`como. Se `StyleId` não estiver definido, fallback para `x:Name`. Ao usar o XAML `x:Name` , é preferível `StyleId`. Observe que esse seletor diferencia maiúsculas de minúsculas.|
|`*`|`*`|Seleciona todos os elementos.|
|`element`|`label`|Seleciona todos os elementos do `Label`tipo, mas não as subclasses. Observe que esse seletor não diferencia maiúsculas de minúsculas.|
|`^base`|`^contentpage`|Seleciona todos os elementos `ContentPage` com como a classe base, `ContentPage` incluindo a si mesmo. Observe que esse seletor não diferencia maiúsculas de minúsculas e não faz parte da especificação de CSS.|
|`element,element`|`label,button`|Seleciona todos `Button` os elementos e `Label` todos os elementos. Observe que esse seletor não diferencia maiúsculas de minúsculas.|
|`element element`|`stacklayout label`|Seleciona todos `Label` os elementos dentro `StackLayout`de um. Observe que esse seletor não diferencia maiúsculas de minúsculas.|
|`element>element`|`stacklayout>label`|Seleciona todos `Label` os elementos `StackLayout` com como um pai direto. Observe que esse seletor não diferencia maiúsculas de minúsculas.|
|`element+element`|`label+entry`|Seleciona todos `Entry` os elementos diretamente após `Label`a. Observe que esse seletor não diferencia maiúsculas de minúsculas.|
|`element~element`|`label~entry`|Seleciona todos `Entry` os elementos precedidos `Label`por um. Observe que esse seletor não diferencia maiúsculas de minúsculas.|
...
> [!TIP]
> Os `StackLayout>ContentView>label.email`seletores podem ser combinados sem limitação, como.
...
As propriedades de CSS a seguir têm suporte do Xamarin. Forms (na coluna **valores, os** tipos são em _itálico_, enquanto os `gray`literais de cadeia de caracteres são):
...
|`flex-basis`|`VisualElement`|_float_ \| float `auto` . \| `initial` Além disso, um percentual no intervalo de 0% a 100% pode ser especificado com o `%` sinal.|`flex-basis: 25%;`|
...
|`font-size`|`Button`, `DatePicker`, `Editor`, `Entry`, `Label`, `Picker`, `SearchBar`, `TimePicker`, `Span`|_double_\| _namedsize_ namedsize \| duplo  `initial` |`font-size: 12;`|
...
Há suporte `color` para os seguintes valores:
...
Há suporte para um, dois, três `thickness` ou quatro valores, cada um separado por um espaço em branco:
...
> [!NOTE]
> Os `thickness` valores de CSS diferem dos valores XAML [`Thickness`](xref:Xamarin.Forms.Thickness) . Por exemplo, em XAML, um valor de `Thickness` dois valores indica a espessura horizontal e vertical, enquanto um `Thickness` valor de quatro valores indica esquerda, em cima, à direita e na espessura inferior. Além disso, os `Thickness` valores XAML são delimitados por vírgula.

### <a name="namedsize"></a>NamedSize

Os valores de não diferenciação `namedsize` de caso a seguir têm suporte:
