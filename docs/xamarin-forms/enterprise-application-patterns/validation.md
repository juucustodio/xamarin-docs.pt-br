...
[![](validation-images/validation.png "Classes de validação no aplicativo móvel eShopOnContainers")](validation-images/validation-large.png "classes de validação no aplicativo móvel eShopOnContainers")
...
Validação é disparada automaticamente sempre que uma propriedade associada é alterado. Por exemplo, quando uma associação bidirecional no `LoginView` define o `UserName` ou `Password` propriedade, a validação é disparada. O exemplo de código a seguir demonstra como isso ocorre:
