...
A `Negotiate` função responde a solicitações da Web com `SignalRConnectionInfo` um objeto que contém `AccessToken` uma propriedade e `Url` uma propriedade. O aplicativo móvel usa esses valores para se registrar no Hub do Signalr. O código a seguir mostra `Negotiate` a função:
...
A `Talk` função responde a solicitações HTTP post que fornecem um objeto Message no corpo da postagem. O corpo da postagem é transformado em um `SignalRMessage` e encaminhado para o Hub do signalr. O código a seguir mostra `Talk` a função:
...
> [!NOTE]
> A `Username` Propriedade no arquivo **Constants.cs** do aplicativo de exemplo usa o valor `RuntimePlatform` do dispositivo como o nome de usuário. Isso torna mais fácil testar dispositivos entre plataformas e identificar qual dispositivo está enviando a mensagem. Em um aplicativo do mundo real, esse valor provavelmente seria um nome de usuário exclusivo, coletado durante um processo de inscrição ou de entrada.
...
O `SendMessageAsync` método`SignalRService` na classe é usado para enviar mensagens aos clientes conectados ao serviço de signaler do Azure. Esse método executa uma solicitação HTTP post para a função **Talk** hospedada no aplicativo Azure functions, incluindo um objeto serializado `Message` em JSON como a carga de postagem. A função **Talk** passa a mensagem para o serviço de signaler do Azure para difusão a todos os clientes conectados. O código a seguir mostra `SendMessageAsync` o método:
...
O código a seguir mostra `ConnectAsync` o método:
...
O `AddNewMessage` método é associado como o manipulador de eventos `ConnectAsync` na mensagem, conforme mostrado no código anterior. Quando uma mensagem é recebida, o `AddNewMessage` método é chamado com os dados da mensagem fornecidos como `JObject`um. O `AddNewMessage` método converte o `JObject`emuma instância da `NewMessageReceived` classee,emseguida,invocaomanipuladorparaseumtiversidoassociado.`Message` O código a seguir mostra `AddNewMessage` o método:
...
Utilizar a classe de serviço signalr no Xamarin. Forms é feito ligando os `SignalRService` eventos de classe `MainPage` na classe code-behind.

O `Connected` evento`SignalRService` na classe é acionado quando uma conexão de signalr é concluída com êxito. O `ConnectionFailed` evento`SignalRService` na classe é acionado quando uma conexão de signalr falha. O `SignalR_ConnectionChanged` método do manipulador de eventos está associado a ambos os `MainPage` eventos no construtor. Esse manipulador de eventos atualiza os Estados de botão conectar e enviar com base `success` no argumento de conexão e adiciona a mensagem fornecida pelo evento à fila de chat usando `AddMessage` o método. O código a seguir mostra `SignalR_ConnectionChanged` o método do manipulador de eventos:
...
O `NewMessageReceived` evento`SignalRService` na classe é acionado quando uma nova mensagem é recebida do serviço de signaler do Azure. O `SignalR_NewMessageReceived` método do manipulador de eventos está associado `NewMessageReceived` ao evento no `MainPage` Construtor. Esse manipulador de eventos converte o `Message` objeto de entrada em uma cadeia de caracteres e o adiciona à fila `AddMessage` de chat usando o método. O código a seguir mostra `SignalR_NewMessageReceived` o método do manipulador de eventos:
...
O `AddMessage` método adiciona uma nova mensagem como um `Label` objeto à fila de chat. O `AddMessage` método é geralmente chamado por manipuladores de eventos de fora do thread da interface do usuário principal, portanto, ele força as atualizações da interface do usuário a ocorrer no thread principal para evitar exceções. O código a seguir mostra `AddMessage` o método:
