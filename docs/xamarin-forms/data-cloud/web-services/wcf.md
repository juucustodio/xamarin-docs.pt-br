...
_O WCF é a estrutura unificada da Microsoft para a criação de aplicativos orientados a serviços. Ele permite que os desenvolvedores criem aplicativos distribuídos seguros, confiáveis, transacionados e interoperáveis. Este artigo demonstra como consumir um serviço de protocolo SOAP (Simple Object Access Protocol) do WCF de um aplicativo Xamarin. Forms._
...
> [!IMPORTANT]
> O suporte à plataforma Xamarin para WCF é limitado a mensagens SOAP codificadas por texto por HTTP/HTTPS usando a classe `BasicHttpBinding`.
>
...
Para cada chamada para *BeginOperationName*, o aplicativo também deve chamar *endoperable* para obter os resultados da operação. O valor de retorno de *Endoperable* é o mesmo tipo retornado pelo método de serviço Web síncrono. Por exemplo, o método `EndGetTodoItems` retorna uma coleção de instâncias de `TodoItem`. O método *endoperation* também inclui um parâmetro `IAsyncResult` que deve ser definido para a instância retornada pela chamada correspondente ao método *BeginOperationName* .

A TPL (biblioteca paralela de tarefas) pode simplificar o processo de consumo de um par de métodos de início/término do APM encapsulando as operações assíncronas no mesmo objeto `Task`. Esse encapsulamento é fornecido por várias sobrecargas do método `TaskFactory.FromAsync`.
...
A classe de proxy gerada fornece a classe `TodoServiceClient`, que é usada para se comunicar com o serviço WCF por HTTP. Ele fornece funcionalidade para invocar métodos de serviço web, como operações assíncronas de um URI identificado a instância de serviço. Para obter mais informações sobre operações assíncronas, consulte [visão geral do suporte assíncrono](~/cross-platform/platform/async.md).

A instância de `TodoServiceClient` é declarada no nível de classe para que o objeto fique por enquanto o aplicativo precisa consumir o serviço WCF, conforme mostrado no exemplo de código a seguir:
...
O aplicativo de exemplo usa a classe `TodoItem` para modelar dados. Para armazenar um item de `TodoItem` no serviço Web, primeiro ele deve ser convertido para o tipo de `TodoItem` gerado pelo proxy. Isso é feito pelo método `ToWCFServiceTodoItem`, conforme mostrado no exemplo de código a seguir:
...
Esse método simplesmente cria uma nova instância de `TodoWCFService.TodoItem` e define cada propriedade para a propriedade idêntica da instância de `TodoItem`.

Da mesma forma, quando os dados são recuperados do serviço Web, eles devem ser convertidos do tipo de `TodoItem` gerado pelo proxy para uma instância do `TodoItem`. Isso é feito com o método `FromWCFServiceTodoItem`, conforme mostrado no exemplo de código a seguir:
...
Esse método simplesmente recupera os dados do tipo de `TodoItem` gerado pelo proxy e os define na instância de `TodoItem` recém-criada.

### <a name="retrieve-data"></a>Recuperar dados

Os métodos `TodoServiceClient.BeginGetTodoItems` e `TodoServiceClient.EndGetTodoItems` são usados para chamar a operação de `GetTodoItems` fornecida pelo serviço Web. Esses métodos assíncronos são encapsulados em um objeto `Task`, conforme mostrado no exemplo de código a seguir:
...
O método `Task.Factory.FromAsync` cria um `Task` que executa o método `TodoServiceClient.EndGetTodoItems` depois que o método `TodoServiceClient.BeginGetTodoItems` é concluído, com o parâmetro `null` indicando que nenhum dado está sendo passado para o delegado `BeginGetTodoItems`. Por fim, o valor da enumeração `TaskCreationOptions` especifica que o comportamento padrão para a criação e execução de tarefas deve ser usado.

O método `TodoServiceClient.EndGetTodoItems` retorna uma `ObservableCollection` de instâncias de `TodoWCFService.TodoItem`, que é então convertida em uma `List` de instâncias de `TodoItem` para exibição.

### <a name="create-data"></a>Criar dados

Os métodos `TodoServiceClient.BeginCreateTodoItem` e `TodoServiceClient.EndCreateTodoItem` são usados para chamar a operação de `CreateTodoItem` fornecida pelo serviço Web. Esses métodos assíncronos são encapsulados em um objeto `Task`, conforme mostrado no exemplo de código a seguir:
...
O método `Task.Factory.FromAsync` cria um `Task` que executa o método `TodoServiceClient.EndCreateTodoItem` depois que o método `TodoServiceClient.BeginCreateTodoItem` é concluído, com o parâmetro `todoItem` sendo os dados passados para o delegado `BeginCreateTodoItem` para especificar o `TodoItem` a ser criado pelo serviço Web. Por fim, o valor da enumeração `TaskCreationOptions` especifica que o comportamento padrão para a criação e execução de tarefas deve ser usado.

O serviço Web gera um `FaultException` se ele falhar ao criar o `TodoItem`, que é manipulado pelo aplicativo.

### <a name="update-data"></a>Atualizar dados

Os métodos `TodoServiceClient.BeginEditTodoItem` e `TodoServiceClient.EndEditTodoItem` são usados para chamar a operação de `EditTodoItem` fornecida pelo serviço Web. Esses métodos assíncronos são encapsulados em um objeto `Task`, conforme mostrado no exemplo de código a seguir:
...
O método `Task.Factory.FromAsync` cria um `Task` que executa o método `TodoServiceClient.EndEditTodoItem` depois que o método `TodoServiceClient.BeginCreateTodoItem` é concluído, com o parâmetro `todoItem` sendo os dados passados para o delegado `BeginEditTodoItem` para especificar o `TodoItem` a ser atualizado pelo serviço Web. Por fim, o valor da enumeração `TaskCreationOptions` especifica que o comportamento padrão para a criação e execução de tarefas deve ser usado.

O serviço Web gera um `FaultException` se ele falhar ao localizar ou atualizar o `TodoItem`, que é manipulado pelo aplicativo.

### <a name="delete-data"></a>Excluir dados

Os métodos `TodoServiceClient.BeginDeleteTodoItem` e `TodoServiceClient.EndDeleteTodoItem` são usados para chamar a operação de `DeleteTodoItem` fornecida pelo serviço Web. Esses métodos assíncronos são encapsulados em um objeto `Task`, conforme mostrado no exemplo de código a seguir:
...
O método `Task.Factory.FromAsync` cria um `Task` que executa o método `TodoServiceClient.EndDeleteTodoItem` depois que o método `TodoServiceClient.BeginDeleteTodoItem` é concluído, com o parâmetro `id` sendo os dados passados para o delegado `BeginDeleteTodoItem` para especificar o `TodoItem` a ser excluído pelo serviço Web. Por fim, o valor da enumeração `TaskCreationOptions` especifica que o comportamento padrão para a criação e execução de tarefas deve ser usado.

O serviço Web gera um `FaultException` se ele falhar ao localizar ou excluir o `TodoItem`, que é manipulado pelo aplicativo.
...
1. **Configure IIS Express para aceitar conexões remotas**. Você pode configurar IIS Express editando o arquivo de configuração para IIS Express em **[diretório de solução]\.vs\config\applicationhost.config**. Localize o elemento `site` com o nome `TodoWCFService`. Ele deve ser semelhante ao seguinte XML:
    ...
    Você precisará adicionar dois elementos de `binding` para abrir a porta 49393 para o tráfego externo e o emulador do Android. A associação usa um formato `[IP address]:[port]:[hostname]` que especifica como IIS Express responderá às solicitações. As solicitações externas terão nomes de host que devem ser especificados como um `binding`. Adicione o seguinte XML ao elemento `bindings`, substituindo o endereço IP pelo seu próprio endereço IP:
    ...
    Após as alterações, o elemento `bindings` deve ser semelhante ao seguinte:
...
1. **Personalizar os dispositivos de ponto de extremidade use para acessar o serviço**. Esta etapa envolve a configuração do aplicativo cliente, em execução em um dispositivo físico ou emulado, para acessar o serviço WCF.
    ...
    Verifique se a propriedade `SoapUrl` no arquivo **Constants.cs** no projeto TodoWCF (portátil) tem valores que estão corretos para sua rede:
