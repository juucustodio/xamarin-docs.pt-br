...
Este exemplo de código recupera o elefante selecionado no momento em `CollectionView` e navega até a rota `elephantdetails`, passando `elephantName` como um parâmetro de consulta. Os parâmetros de consulta serão codificados em URL para navegação, portanto, "Elefante Indiano" se tornará "Elefante%20Indiano".
