# [Plataforma cruzada](index.yml)
## [Introdução](get-started/index.md)
### [Requisitos](get-started/requirements.md)
### [Instalação](get-started/installation/index.md)
#### [Instalando o Xamarin no Visual Studio no Windows](get-started/installation/windows.md)
#### [Instalando o Visual Studio para Mac](/visualstudio/mac/installation/)
#### [Instruções de configuração de firewall do Xamarin](get-started/installation/firewall.md)
#### [Desinstalando o Xamarin](get-started/installation/uninstalling-xamarin.md)
### [Introdução ao desenvolvimento móvel](get-started/introduction-to-mobile-development.md)
### [Introdução ao SDLC móvel](get-started/introduction-to-mobile-sdlc.md)
## [Conceitos básicos de aplicativo](app-fundamentals/index.md)
### [Acessibilidade](app-fundamentals/accessibility.md)
### [Compilar aplicativos de plataforma cruzada](app-fundamentals/building-cross-platform-applications/index.md)
#### [Visão geral](app-fundamentals/building-cross-platform-applications/part-0-overview.md)
#### [Parte 1 – Noções básicas sobre a plataforma Xamarin Mobile](app-fundamentals/building-cross-platform-applications/part-1-understanding-the-xamarin-mobile-platform.md)
#### [Parte 2 – Arquitetura](app-fundamentals/building-cross-platform-applications/part-2-architecture.md)
#### [Parte 3 – Configuração de uma solução de plataforma cruzada do Xamarin](app-fundamentals/building-cross-platform-applications/part-3-setting-up-a-xamarin-cross-platform-solution.md)
#### [Parte 4 – Lidar com várias plataformas](app-fundamentals/building-cross-platform-applications/part-4-platform-divergence-abstraction-divergent-implementation.md)
#### [Parte 5 – Estratégias práticas de compartilhamento de código](app-fundamentals/building-cross-platform-applications/part-5-practical-code-sharing-strategies.md)
#### [Parte 6 – Teste e aprovações da App Store](app-fundamentals/building-cross-platform-applications/part-6-testing-and-app-store-approvals.md)
#### [Estudo de caso: Tasky](app-fundamentals/building-cross-platform-applications/case-study-tasky.md)
### [Opções de compartilhamento de código](app-fundamentals/code-sharing.md)
### [Bibliotecas de classes portáteis](app-fundamentals/pcl.md)
### [Projetos compartilhados](app-fundamentals/shared-projects.md)
### [.NET Standard](app-fundamentals/net-standard.md)
### [Pacotes NuGet (automatizados)](app-fundamentals/nuget-multiplatform-libraries/index.md)
#### [Projetos de biblioteca existentes](app-fundamentals/nuget-multiplatform-libraries/existing-library.md)
#### [Nova biblioteca multiplataforma](app-fundamentals/nuget-multiplatform-libraries/single-codebase.md)
#### [Novas bibliotecas específicas da plataforma](app-fundamentals/nuget-multiplatform-libraries/platform-specific.md)
#### [Guia de metadados](app-fundamentals/nuget-multiplatform-libraries/metadata.md)
### [Pacotes NuGet (Manual)](app-fundamentals/nuget-manual.md)
### [Localização](app-fundamentals/localization.md)
### [TLS (Transport Layer Security)](app-fundamentals/transport-layer-security.md)
## [Recursos da plataforma](platform/index.md)
### [Visão geral do suporte assíncrono](platform/async.md)
### [Recursos de linguagem C# 6](platform/csharp-six.md)
### [F#](platform/fsharp/index.md)
#### [Introdução](platform/fsharp/overview.md)
#### [Exemplos do F#](platform/fsharp/samples.md)
### [Visual Basic.NET](platform/visual-basic/index.md)
#### [Xamarin iOS e Android](platform/visual-basic/native-apps.md)
#### [Xamarin.Forms](platform/visual-basic/xamarin-forms.md)
### [Modelos HTML do Razor](platform/razor-html-templates/index.md)
## [Serviços de dados e de nuvem](data-cloud/index.md)
### [Serviços conectados](data-cloud/connected-services.md)
### [Azure Active Directory](data-cloud/active-directory/index.md)
#### [Introdução](data-cloud/active-directory/get-started/index.md)
##### [Etapa 1. Registrar](data-cloud/active-directory/get-started/register.md)
##### [Etapa 2. Configurar](data-cloud/active-directory/get-started/configure.md)
#### [Acessar a API do Graph](data-cloud/active-directory/graph.md)
### [Aplicativos Móveis do Azure](data-cloud/mobile-apps.md)
### [Introdução aos serviços Web](data-cloud/web-services/index.md)
#### [Passo a passo: como trabalhar com WCF](data-cloud/web-services/walkthrough-working-with-wcf.md)

## [Implantação e Teste](deploy-test/index.md)
### [Configuração personalizada do vinculador](deploy-test/linker.md)
### [Desempenho](deploy-test/memory-perf-best-practices.md)
### [Instalação do NUnit 2.6.4 por meio do NuGet](deploy-test/installing-nunit-using-nuget.md)
### [Depuração de vários processos](deploy-test/multi-process-debugging.md)
## [Plataforma Apple](macios/index.md)
### [Gerenciamento de contas da Apple](macios/apple-account-management.md)
### [API Unificada](macios/unified/index.md)
#### [Visão geral](macios/unified/overview.md)
#### [Atualizar aplicativos existentes](macios/unified/updating-apps.md)
#### [Atualizar aplicativos iOS existentes](macios/unified/updating-ios-apps.md)
#### [Atualizar aplicativos Mac existentes](macios/unified/updating-mac-apps.md)
#### [Atualizar aplicativos Xamarin.Forms existentes](macios/unified/updating-xamarin-forms-apps.md)
#### [Migrar uma associação para a API unificada](macios/unified/update-binding.md)
#### [Dicas para atualizar o código para a API unificada](macios/unified/updating-tips.md)
### [Associação do Objective-C](macios/binding/index.md)
#### [Visão geral](macios/binding/overview.md)
#### [Associação de bibliotecas de Objective-C](macios/binding/objective-c-libraries.md)
#### [Referência de tipos de associação](macios/binding/binding-types-reference.md)
#### [Objective Sharpie](macios/binding/objective-sharpie/index.md)
##### [Introdução](macios/binding/objective-sharpie/get-started.md)
##### [Ferramentas e comandos](macios/binding/objective-sharpie/tools.md)
##### [Recursos](macios/binding/objective-sharpie/platform/index.md)
###### [ApiDefinitions e StructsAndEnums](macios/binding/objective-sharpie/platform/apidefinitions-structsandenums.md)
###### [Estruturas de associação nativa](macios/binding/objective-sharpie/platform/native-frameworks.md)
###### [Verificar atributos](macios/binding/objective-sharpie/platform/verify.md)
##### [Exemplos](macios/binding/objective-sharpie/examples/index.md)
###### [Xcode](macios/binding/objective-sharpie/examples/xcode.md)
###### [CocoaPod](macios/binding/objective-sharpie/examples/cocoapod.md)
###### [Avançado (manual)](macios/binding/objective-sharpie/examples/advanced.md)
##### [Histórico de versões](macios/binding/objective-sharpie/releases.md)
#### [Solução de problemas](macios/binding/troubleshooting.md)
### [Referências nativas](macios/native-references.md)
### [Tipos nativos](macios/nativetypes.md)
### [Considerações sobre a plataforma de 32 e 64 bits](macios/32-and-64/index.md)
#### [Atualizar os aplicativos unificados do Xamarin.Mac para 64 bits](macios/32-and-64/mac-64-bit.md)
### [Trabalhar com tipos nativos em aplicativos de plataforma cruzada](macios/native-types-cross-platform.md)
### [Pilha de HttpClient e seletor de implementação SSL/TLS](macios/http-stack.md)
## [Recursos internos](internals/index.md)
### [Assemblies disponíveis](internals/available-assemblies.md)
## [Solução de problemas](troubleshooting/index.md)
### [Perguntas frequentes](troubleshooting/questions/index.md)
#### [Posso usar o Visual Studio 2017 Release Candidate com Xamarin?](troubleshooting/questions/visualstudio-2017-rc.md)
#### [Como posso exibir as bibliotecas com suporte em um PCL?](troubleshooting/questions/pcl-support-libraries.md)
#### [API de reflexão de PCL](troubleshooting/questions/pcl-reflection.md)
#### [Estudo de caso de PCL: como posso resolver problemas relacionados a System.Diagnostics.Tracing para o pacote NuGet de Fluxo de Dados de TPL da Microsoft?](troubleshooting/questions/pcl-case-study.md)
#### [Como posso atualizar o NuGet?](troubleshooting/questions/nuget-update.md)
#### [Como fazer downgrade de um pacote NuGet?](troubleshooting/questions/nuget-package-downgrade.md)
#### [Erro de pacotes ausentes após atualizar os pacotes do Nuget](troubleshooting/questions/nuget-packages-missing.md)
#### [Unificar os componentes do Google Play Services e o NuGet](troubleshooting/questions/gps-components-nuget.md)
#### [Onde meus componentes são armazenados no meu computador?](troubleshooting/questions/component-storage.md)
#### [Onde posso encontrar informações e logs da minha versão?](troubleshooting/questions/version-logs.md)
#### [Quando e como eu devo criar um relatório de bugs?](troubleshooting/questions/howto-file-bug.md)
#### [Por que não há suporte para Jenkins no Xamarin?](troubleshooting/questions/xamarin-jenkins.md)
#### [Quais configurações do projeto são necessárias para o depurador?](troubleshooting/questions/debugger-settings.md)
### [Atualizar referências de componentes para NuGet](troubleshooting/component-nuget.md)
### [Opções com suporte](troubleshooting/support-options.md)
### [Perguntas sobre licença herdadas](troubleshooting/legacy-licenses/index.md)
#### [Gerenciamento de equipe da plataforma Xamarin](troubleshooting/legacy-licenses/team-management.md)
#### [Como posso alterar o endereço de email da minha conta?](troubleshooting/legacy-licenses/change-email.md)
#### [Como ressincronizar manualmente as licenças do Xamarin?](troubleshooting/legacy-licenses/resync-licenses.md)
#### [Alguns erros de licenciamento específicos](troubleshooting/legacy-licenses/licensing-errors.md)
#### [Por que não posso efetuar login no Xamarin no Visual Studio ou no Visual Studio para Mac?](troubleshooting/legacy-licenses/login-troubleshooting.md)
## [Amostras](samples/index.yml)
