---
title: Resolver problemas relacionados ao Tracing e fluxo de dados TPL
description: 'Estudo de caso PCL: como resolver problemas relacionados a Tracing para o pacote do NuGet do fluxo de dados do Microsoft TPL?'
ms.prod: xamarin
...
> [!IMPORTANT]
> Esse exemplo específico de `System.Diagnostic.Tracing` não gera erros por padrão nas versões mais recentes do Xamarin. Enquanto a solução alternativa sugerida ainda funcionarão, observe que alguns dos erros mencionados na seção "camadas de erros" foram corrigidos.
> Além disso, você deve observar que .NET padrão agora é a melhor maneira de implementar as APIs .NET de plataforma cruzada.

## <a name="summary"></a>Resumo

Xamarin e xamarin não implementam 100% de cada perfil PCL permitem como referências. Para conveniência prática no Visual Studio para Mac, o Visual Studio e o NuGet package manager, Xamarin projetos permitem o uso de vários perfis que têm apenas _incompleta_ implementações. Por exemplo, nem xamarin nem xamarin atualmente inclui uma implementação completa dos tipos "Tracing" PCL namespace. Essa limitação resulta em três camadas de erros ao tentar usar o padrão `portable-net45+win8+wpa81` versão do pacote do NuGet do fluxo de dados TPL do Microsoft.
...
(Isso evita a todos os três camadas de erros e funciona para todas as versões recentes do Xamarin.)
...
O `portable-net45+win8+wp8+wpa81` não faz referência a versão da biblioteca **System.Diagnostics.Tracing.dll** _em todos os_, por isso evita completamente todos os três níveis de problemas.
...
### <a name="will-it-help-if-i-manually-add-a-reference-to-the-systemdiagnosticstracing-facade-assembly"></a>Ele ajudará se adicionar manualmente uma referência ao assembly de fachada "Tracing"?

_Em particular é pode resolver o problema usando estas 2 etapas?_
...
2. _Adicione uma referência ao assembly fachada no projeto de aplicativo xamarin ou xamarin._
...
### <a name="can-i-use-the-systemdiagnosticstracing-nuget-packagehttpswwwnugetorgpackagessystemdiagnosticstracing-to-solve-the-problem"></a>É possível usar o [pacote NuGet Tracing](https://www.nuget.org/packages/System.Diagnostics.Tracing/) para resolver o problema?

#### <a name="answer"></a>Resposta

Não, o pacote do NuGet 3.0 "Tracing" só inclui implementações específico da plataforma para "DNXCore50" e "netcore50". Ele explicitamente _omite_ implementações para xamarin ("MonoAndroid") e xamarin ("MonoTouch" e "xamarinios"). Isso significa que terá uma instalação do pacote _nenhum efeito_ para projetos xamarin e xamarin. O pacote NuGet pressupõe que ambas as plataformas fornecem seus _próprio_ implementação dos tipos. Esse pressuposto é "correto" no sentido de que tem Mono _um_ implementação do namespace, mas como discutido em pontos \#2 e \#3 de "Detalhes sobre as três camadas de erros" acima, o implementação está incompleta no momento. Para que a correção apropriada para a equipe Mono resolver [Bug 27337](https://bugzilla.xamarin.com/show_bug.cgi?id=27337) e [Bug 34890](https://bugzilla.xamarin.com/show_bug.cgi?id=34890).
